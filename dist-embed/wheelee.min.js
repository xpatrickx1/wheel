(function() {
  "use strict";
  function createWheel(options, widgetId) {
    ...
    if (showOpenButton) {
      ...
      const openDelay = options.autoOpenDelay * 1e3;
      setTimeout(() => {
        wheelWrap.classList.remove("_hidden");
        wheelWrap.classList.add("_active");
    ...
    function updateScreenWidth() {
      ...
      size = screenWidth2 <= 550 ? screenWidth2 : 550;
      canvas.width = size;
      canvas.height = size;
      drawWheel();
    }
    ...
    const MIN_ANGLE_CHANGE = 3e-3;
    function startIdleRotation(speed = 2e-4) {
      stopIdleRotation();
      ...
      function tick(now) {
        ...
        if (angleDiff >= MIN_ANGLE_CHANGE || angleDiff > Math.PI * 1.9) {
          drawWheel();
    ...
    function wrapText(ctx2, text, x, y, maxWidth, lineHeight) {
      ...
      lines.forEach((l, i) => {
        ctx2.fillText(l, x + 40, y + offsetY + i * lineHeight);
      });
    }
    function drawWheel() {
      if (!ctx) return;
      ctx.clearRect(0, 0, canvas.width, canvas.height);
      activeBonuses.forEach((prize2, i) => {
    ...
    function spin() {
      ...
      const targetAngle = 2 * Math.PI * 5 + (targetIndex * sliceAngle + sliceAngle / 2) + randomOffset;
      const duration = 5e3;
      const start = performance.now();
      const initialRotation = rotation;
      function easeInOutShifted(t, bias = 0.3) {
        if (t < bias) {
          return Math.pow(t / bias, 2);
        } else {
        ...
        if (!prize) {
          let validateInput = function() {
            if (collectData === "email") {
              ...
              console.log(telRegex.test(input.value.trim()));
    ...
    `;
      const container2 = document.getElementById("wheelee-container") || document.body;
      ...
      for (let i = 0; i < 180; i++) {
        ...
        particles.push({
          x: canvas2.width * (0.3 + Math.random() * 0.4),
          y: canvas2.height * (0.4 + Math.random() * 0.3),
      ...
      function animate() {
        ...
        for (const p of particles) {
          ...
          if (p.life > 0 && p.y < canvas2.height + 50) {
            alive = true;
            ctx2.save();
            ctx2.globalAlpha = p.alpha * 0.9;
    ...
    function formatPhone(digits) {
      ...
      if (val.length >= 3) result += " (" + val.substring(2, 5);
      if (val.length >= 6) result += ") " + val.substring(5, 8);
      if (val.length >= 9) result += "-" + val.substring(8, 10);
      if (val.length >= 11) result += "-" + val.substring(10, 12);
      return result;
    }
  ...
  (function() {
    async function init() {
      ...
      fetch(`${API_BASE}/functions/v1/get-widget/${widgetId}`).then((r) => {
